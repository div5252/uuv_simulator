...
import rospy
import sys
from uuv_control_msgs.srv import InitStraightLineTrajectory
...
if __name__ == '__main__':
    print('Starting the straight line trajectory creator')
    rospy.init_node('start_straight_line_trajectory')
    ...
    try:
        rospy.wait_for_service('start_straight_line_trajectory', timeout=20)
    except rospy.ROSException:
        print('Service not available! Closing node...')
        sys.exit(-1)

    try:
        traj_gen = rospy.ServiceProxy('start_straight_line_trajectory', InitStraightLineTrajectory)
