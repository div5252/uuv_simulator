...
import rospy
import sys
from uuv_control_msgs.srv import InitSinusoidalCurveTrajectory
...
if __name__ == '__main__':
    print('Starting the sinusoidal curve trajectory creator')
    rospy.init_node('start_sinusoidal_curve_trajectory')
    ...
    param_labels = ['start', 'amplitude', 'frequency', 'phase', 'cycles', 
                    'n_points', 'heading_offset', 'duration', 'max_forward_speed']
    params = dict()
    ...
    if len(params['start']) != 3:
        print('Start of the curve must have 3 components (x, y, z)')
        sys.exit(-1)
    ...
    try:
        rospy.wait_for_service('start_sinusoidal_curve_trajectory', timeout=20)
    except rospy.ROSException:
        print('Service not available! Closing node...')
        sys.exit(-1)

    try:
        traj_gen = rospy.ServiceProxy('start_sinusoidal_curve_trajectory', InitSinusoidalCurveTrajectory)
